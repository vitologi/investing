...
import FirstStepRu from '../first-steps-content.ru';
...
describe('FirstStepEn', ()=>{
  ...
  test('straightforward way', async ()=>{
    const {getByTestId, queryByTestId} = render(<FirstStepRu onFinish={onFinish}/>);
  ...
  test('back way', async ()=>{
    const {getByTestId} = render(<FirstStepRu onFinish={onFinish}/>);
  ...
  test('download transactions', async ()=>{
    ...
    const {getByTestId, getByRole} = render(<FirstStepRu onFinish={onFinish}/>);
